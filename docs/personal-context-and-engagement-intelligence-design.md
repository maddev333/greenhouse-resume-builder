...
> demo personas) has been **removed** from the implementation of the shared engagements capability,
> and retrieval there has moved to a multi-index schema registry with an added grounding-only mode.
> References below to the shared store's tenant/ACL/sensitivity trim no longer reflect the code; the
> user-scoped boundary this document proposes is unaffected. See [`README.md`](../README.md) and
...
- **`engagement-intelligence/ARCHITECTURE.md` §5.4 "Personal notes (stretch)"** — _"a separate MCP
  client that authenticates with Entra ID, decoupled from Keycloak (no cross-IdP brokering)… the
  notes store must be one the user is personally authorized to read."_
- **`engagement-intelligence/seed/schema.ts` — `Preferences`** — a **runtime** personalization input
  the orchestrator forwards to sub-agents to _rank and filter_ candidates, which **"NEVER widens
...
runtime signals that tune _that user's_ menu and pre-briefs, consumed at request time, never written
...
context re-ranks _within_ the leader / duration / extension options that flow produces — it never
...
- Feed **runtime personalization** signals into the planner so _that user's_ itinerary is tailored
...
- **fit scheduling** to the user's real availability and existing trips ("you're already going
  there" gets stronger when it's genuinely on _their_ calendar),
...
withheld. The trim remains the sole authority for _what may be seen_; personal context influences
only _how it's ordered and framed_ for this user.
...
- **Deterministic gating.** Any logic that _gates_ what is surfaced (consent checks, access, tenancy)
  is deterministic — consistent with the repo rule "deterministic core, LLM only at the edges."
  LLM/heuristic inference may compute _derived facts_, but must not be the authority for access.
