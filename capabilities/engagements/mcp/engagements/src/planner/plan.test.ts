...
function route(offSite: number, totalTravelMins: number): RouteResult {
  return {
    order: [
      ...
      ...Array.from({ length: offSite }, (_, i) => ({
        id: `o${i}`,
        location: RESTON,
        kind: "off-site" as const,
      })),
    ],
    legs: [],
    totalMi: 0,
...
test("talkingPointsFor: an approved message supplies the intended points", () => {
  ...
  const m: Message = {
    ...
    status: "approved",
    intendedPoints: [
      "Multi-year contracting stability is coming",
      "Prioritize supply-chain onshoring for munitions",
    ],
  };
...
test("talkingPointsFor: no approved message → graceful coordinate-with-owner fallback", () => {
  const t3: Topic = {
    ...
    domain: "non-technical",
    smeAreas: [],
    ownerOrg: "Army Applications Lab",
...
const T1: Topic = {
  ...env,
  id: "T1",
  name: "Defense industrial base (DIB) resilience",
  domain: "non-technical",
  smeAreas: ["industrial-base"],
  ownerOrg: "ASA(ALT)",
  approvedMessageId: "M-T1-v2",
};
const T3: Topic = {
  ...env,
  id: "T3",
  name: "Defense innovation & startups",
  domain: "non-technical",
  smeAreas: ["innovation/startups"],
  ownerOrg: "Army Applications Lab",
  approvedMessageId: null,
};
const M_T1: Message = {
  ...
  intendedPoints: [
    "Multi-year contracting stability is coming",
    "Prioritize supply-chain onshoring for munitions",
    "No commitments on specific program dollars",
  ],
};

const leaderL1: Leader = {
  ...env,
  id: "L1",
  name: "MG D. Whitfield",
  role: "ASA(ALT) — MG",
  ...
  level: "L4",
  homeBase: DC,
  availability: [{ start: "2025-10-06", end: "2025-10-20" }],
  daysAwayBudget: 10,
};

const meridian: Contact = {
  ...env,
  id: "C3",
  ...
  level: "L3",
  location: RESTON,
  relationshipOwnerLeaderIds: ["L1"],
  strategicValue: 4,
  status: "active",
  lastInteractionDate: "2024-01-01",
};
const startup: Contact = {
  ...env,
  id: "P9",
  name: "NovaLift",
  type: "company",
...
function offer() {
  return extensionOptions({
    ...
    onSiteDays: 0,
    contactsById: new Map<string, Contact>([
      ["C3", meridian],
      ["P9", startup],
    ]),
    topics: [T1, T3],
...
test("extensionOptions: unlocks a sectored entity on its topic WITH approved talking points", () => {
  ...
  assert.ok(
    m.talkingPoints.includes("Multi-year contracting stability is coming"),
  );
...
test("extensionOptions: every option carries talking points and is ranked by marginal ROI", () => {
  ...
  for (let i = 1; i < opts.length; i++)
    assert.ok(opts[i - 1].marginalRoi >= opts[i].marginalRoi);
});
...
test("durationOptions: always offers a core tier; extended appears only when it adds stops", () => {
  ...
  if (opts.length === 2)
    assert.ok(
      opts[1].days >= opts[0].days,
      "extended is at least as long as core",
    );
});
...
test("planOptions: NCR in AUSA week yields survey + leader + duration + extension menus", () => {
  ...
  // B — survey
  assert.ok(
    res.areaSurvey.some((t) => t.topicId === "T1"),
  ...
  for (const target of [
    ...
    assert.ok(
      cats.includes(target as (typeof cats)[number]),
      `NCR breakdown must always report ${target}`,
    );
  }
  const congressional = res.categoryBreakdown.find(
    (c) => c.category === "congressional",
  )!;
  assert.ok(
    congressional.total >= 2,
    "NCR has a Congressional footprint (C31/C32)",
  );
  // each duration option reports its own audience mix
  for (const d of res.durationOptions)
    assert.equal(typeof d.categoryCounts, "object");
});
