...
import { valueNorm, stalenessNorm, topicRelevance, suggestionScore } from './score';
...
test('suggestionScore(active) = staleness × value × topic', () => {
  const c = baseContact({ strategicValue: 4, topicIds: ['T3'], lastInteractionDate: '2024-11-15' });
...
test('suggestionScore(prospect) omits staleness (initiate path)', () => {
  const p = baseContact({ status: 'prospect', strategicValue: 4, topicIds: ['T3'], lastInteractionDate: undefined });
...
test('a very stale, high-value, on-topic active contact outranks a fresh one', () => {
  const stale = baseContact({ lastInteractionDate: '2024-11-15', strategicValue: 4 });
  const fresh = baseContact({ lastInteractionDate: '2025-09-20', strategicValue: 4 });
