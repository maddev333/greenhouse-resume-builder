...
import { categoryBreakdown, categoryCountsForStops, summarizeCategoryCounts } from './categories';

const env = { tenantId: 'army', createdAt: '2025-01-01' };
const DC: GeoPoint = { city: 'Washington', state: 'DC', lat: 38.9072, lng: -77.0369 };
...
test('categoryBreakdown: always emits the four target audiences, even at zero (coverage gap is explicit)', () => {
  ...
  for (const cat of ['congressional', 'academia', 'industry', 'army-internal'] as const) {
  ...
  assert.match(byCat.get('congressional')!.reason, /no Congressional engagements/);
});

test('categoryBreakdown: buckets by audience, excludes out-of-radius, and flags stale', () => {
  const contacts = [
    contact({ id: 'HILL', sector: 'congressional', lastInteractionDate: '2024-06-01' }), // stale
    contact({ id: 'UNI', sector: 'academic', lastInteractionDate: '2025-09-20' }), // fresh
    contact({ id: 'PRIME', sector: 'industry', lastInteractionDate: '2024-01-01' }), // stale
  ...
  const breakdown = categoryBreakdown({ centroid: DC, radiusMi: 75, contacts, cfg: CFG });
  const byCat = new Map(breakdown.map((c) => [c.category, c]));
  assert.equal(byCat.get('congressional')!.total, 1, 'the far Hill contact is excluded');
  assert.equal(byCat.get('congressional')!.staleCount, 1);
  assert.equal(byCat.get('academia')!.staleCount, 0, 'the fresh university is not stale');
...
test('categoryBreakdown: itineraryContactIds drive coverage + the gap flag', () => {
  ...
  assert.equal(byCat.get('congressional')!.covered, false, 'congressional is present but unreached → gap');
...
test('categoryCountsForStops + summarizeCategoryCounts: audience mix for a stop set', () => {
  ...
  const counts = categoryCountsForStops([{ contactId: 'A' }, { contactId: 'B' }, { contactId: 'C' }], byId);
...
test('categoryBreakdown: the real-seed NCR identifies engagements across all four audiences', () => {
  ...
  const byCat = new Map(breakdown.map((c) => [c.category, c]));
  assert.ok((byCat.get('congressional')?.total ?? 0) >= 2, 'C31/C32 give the NCR a Congressional footprint');
  assert.ok((byCat.get('army-internal')?.total ?? 0) >= 2, 'C33/C34 (+APG) give an Army-internal footprint');
  assert.ok((byCat.get('industry')?.total ?? 0) >= 1, 'the NCR has Industry engagements');
