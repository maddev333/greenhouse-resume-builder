...
const MAPS_KEY =
  ((import.meta as { env?: Record<string, string> }).env
    ?.VITE_AZURE_MAPS_KEY as string) || "";
const MAPS_DOMAIN =
  ((import.meta as { env?: Record<string, string> }).env
    ?.VITE_AZURE_MAPS_DOMAIN as string) || "atlas.microsoft.com";
...
const DEMO_PAYLOAD: TripMapPayload = {
  ...
  origin: {
    id: "event:E-AUSA",
    label: "AUSA Annual Meeting",
    lat: 38.9037,
    lng: -77.0229,
    kind: "origin",
    detail: "Washington, DC · 2025-10-13→2025-10-15",
  },
  stops: [
    {
      id: "C1",
      label: "Dr. Alan Pierce",
      lat: 38.9037,
      lng: -77.0229,
      kind: "on-site",
      detail: "re-engage · initiate · val 5 · score 0.81",
    },
    {
      id: "C2",
      label: "COL (R) Diane Fox",
      lat: 38.8816,
      lng: -77.091,
      kind: "off-site",
      detail: "re-engage · STALE · val 4 · score 0.74",
    },
    {
      id: "C3",
      label: "Sam Ortega (Anduril)",
      lat: 39.529,
      lng: -76.162,
      kind: "off-site",
      detail: "initiate · val 4 · score 0.66",
    },
    {
      id: "C4",
      label: "Redstone PM",
      ...
      lng: -86.6689,
      kind: "off-site",
      detail: "re-engage · STALE · val 5 · score 0.63",
    },
  ],
  legs: [
    {
...
function MapView({ payload }: { payload: TripMapPayload }) {
  ...
  useEffect(() => {
    ...
    map.events.add("error", (e) =>
      setMapError(String((e as { message?: string }).message ?? "map error")),
    );
    return () => {
      setReady(false);
  ...
  if (mapError)
    return (
      <SchematicView
        payload={payload}
        note={`Map unavailable (${mapError}). Showing the itinerary as a list.`}
      />
    );
      ...
      <ol style={{ paddingLeft: 20, margin: 0 }}>
        {rows.map((p, i) => (
          <li key={p.id} style={{ marginBottom: 8 }}>
            ...
            {p.detail && (
              <div className="muted" style={{ marginLeft: 18 }}>
                {p.detail}
              </div>
            )}
          </li>
  ...
  return (
    <main style={{ maxWidth: 1040, margin: "1rem auto", padding: "0 1rem" }}>
      <header style={{ marginBottom: 12 }}>
        ...
        >
          <span>
            {stopCount} stop(s) · {offSite} off-site
          </span>
          {typeof payload.totalMi === "number" && (
            <span>{payload.totalMi} mi travel</span>
          )}
          {typeof payload.roiScore === "number" && (
            <span style={{ color: payload.overBudget ? "#f85149" : undefined }}>
              ROI {payload.roiScore}
              {payload.overBudget ? " · OVER BUDGET" : ""}
            </span>
          )}
        </div>
      </header>
      {MAPS_KEY ? (
        <MapView payload={payload} />
      ) : (
        <SchematicView
          payload={payload}
          note="No Azure Maps key baked into this build — showing the itinerary as a list. Set AZURE_MAPS_KEY in the repo-root .env and rebuild to see the map."
        />
      )}
    </main>
  );
...
function tripMapFromResult(
  result: CallToolResult | null,
): TripMapPayload | null {
  const sc = result?.structuredContent as
    | { tripMap?: TripMapPayload }
    | undefined;
...
function ConnectedApp() {
  ...
  if (error)
    return <Notice>Could not connect to the MCP host: {error.message}</Notice>;
  if (!app) return <Notice>Connecting to host…</Notice>;
  if (!payload)
    return (
      <Notice>
        Waiting for an itinerary — run <code>build_itinerary</code> to plot the
        trip.
      </Notice>
    );
