...
| Tool                 | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search_grounding`   | **Grounded RAG** — ranked passages from a document/chunk index (hybrid BM25 + vector, semantic reranking where configured), returned with citation metadata (`id`, `title`, `url`, `parentId`). Inputs: `query`, `top` (default 8), `maxPerParent` (default 1), optional OData `filter`. Registration is conditional — see above.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
The MCP's deterministic planner remains unavailable in `grounding` mode. The engagements **agent**
can nevertheless synthesize a cited `documentPlan` by loading its trusted trip-planning skill and
using only passages returned by `search_grounding`; this does not add or emulate structured MCP
contact/event tools.
...
`search_grounding` over-fetches, then limits passages **by parent document** before taking the top
N. The default `maxPerParent=1` prevents one long PDF from occupying every result slot; batched JSON
or CSV record lookups can raise the cap to return several distinct records from the same source.
