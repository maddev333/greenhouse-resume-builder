...
The shared library [`mcp-core`](./mcp-core) provides MCP-server, identity/token, and claims-based
security helpers. Agent execution and governance live in the Python package inside `agent/`.
...
| [mcp-core](./mcp-core) | — (shared library: MCP, identity, and security helpers) | — | — |
...
- `AGT_ENABLED` (default `true`) and `AGT_POLICY_PATH` (default `governance/policy.yaml`).
