 ...
 * (`tenantId`, `createdAt`) so records satisfy the shared domain types. This stands in for the
...
export function loadDataset(opts: LoadOptions = {}): Dataset {
  ...
  const sh = (iso?: string): string | undefined => (iso ? shiftDateByMonths(iso, shift) : iso);
  const shRange = (r: DateRange): DateRange => ({ start: shiftDateByMonths(r.start, shift), end: shiftDateByMonths(r.end, shift) });
  const envelope = { tenantId, createdAt: today };

  const topics = readJson<Topic>(dir, 'topics.json').map((t) => ({ ...envelope, ...t }));
  ...
  const engagements = readJson<Engagement>(dir, 'engagements.json').map((g) => ({
    ...envelope,
    ...g,
    date: shiftDateByMonths(g.date, shift),
  }));

  const afteractions = readJson<AfterActionNote>(dir, 'afteractions.json').map((a) => ({
    ...envelope,
    ...a,
  }));
